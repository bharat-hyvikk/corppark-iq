...
<body class="bg-[#e0e0e0] text-black">
    ...
    @if (session('noVehicle'))
        <div
            class="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded text-center text-sm mb-2 custom-alert">
            {{ session('noVehicle') }}
        </div>
    @endif

    @if (session('success'))
        <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-2 rounded text-center text-sm mb-2 custom-alert"
            id="errorMsgCustom">
            {{ session('success') }}
        </div>
    @endif
        ...
        <div class="w-11/12 lg:w-9/12 mx-auto py-10">
            ...
            <form class="bg-white shadow-lg rounded-xl p-6 md:p-10 w-full lg:w-2/3 mx-auto space-y-6"
                ...
                <textarea rows="5" placeholder="Your Message" name="message"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('message') }}
                </textarea>
                @error('message')
                    <div class="text-red-500">{{ $message }}</div>
                @enderror
