<table class="table align-items-center mb-0">
    <thead class="bg-gray-100">
        <tr>
            ...
            @if (auth()->user()->user_type == '1' || auth()->user()->can('vehicle.update') || auth()->user()->can('vehicle.delete'))
        ...
        @foreach ($vehicles as $vehicle)
            <tr id="vehicle-{{ $vehicle->id }}">
                ...
                <td class="text-center align-middle">
                    @if (auth()->user()->user_type == '1' || auth()->user()->can('vehicle.update') || auth()->user()->can('vehicle.delete'))
                        @if (auth()->user()->user_type == '1' || auth()->user()->can('vehicle.update'))
                        ...
                        @endif
                        @if (auth()->user()->user_type == '1' || auth()->user()->can('vehicle.delete'))
                            <a href="javascript:;" data-id="{{ $vehicle->id }}"
                                class="text-secondary font-weight-bold text-xs mx-2 delete-item"
                                data-bs-toggle="tooltip" data-bs-title="Delete Vehicle" data-bs-toggle="modal"
                                data-bs-target="#deleteaddModal" data-name="{{ $vehicle->vehicle_number }}">
