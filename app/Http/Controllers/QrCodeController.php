    ...
    public function index(Request $request)
    {
        ...
        if ($id) {
            ...
            if ($request->ajax()) {
                ...
                if ($filter === 'hasQr') {
                    $qrs = $qrs->whereHas('qrCode', function ($query) use ($office) {
                        $query->where('office_id', $office->id);
                    });
                } elseif ($filter === 'noQr') {
                    $qrs = $qrs->doesntHave('qrCode', 'and', function ($query) use ($office) {
                        $query->where('office_id', $office->id);
                    });
                }
                // Paginate results
            ...
            $qrs = $qrs->paginate(30);
            return view('qr-codes.qr_code', compact("offices", "qrs", "vehicleCount", "totalBrandedQrVehiclesCount"));
        }
